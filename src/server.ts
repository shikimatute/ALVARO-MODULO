      ...
      if (data.event_datasets?.["control"]) {
        if (data.event_datasets?.["control"]?.["event_datasets"]) {
          if (data.event_datasets?.["control"]?.["event_datasets"]?.["zonas"]) {
            ...
            if (
              data.event_datasets?.["control"]?.["event_datasets"]?.["planificacion "]?.["proyecto"]
            ) {
              const proyecto =
                data.event_datasets?.["control"]?.["event_datasets"]?.["planificacion "]?.[
                  "proyecto"
                ];
              ...
              res.status(500).json({
                message:
                  "Error al llamar a la API externa. no se entregaron los datos correspondientes. - BLOQUE 5",
              });
            }
          } else {
            res.status(500).json({
              message:
                "Error al llamar a la API externa. no se entregaron los datos correspondientes. - BLOQUE 1",
            });
          }
        } else {
          res.status(500).json({
            message:
              "Error al llamar a la API externa. no se entregaron los datos correspondientes. - BLOQUE 2",
          });
        }
      } else {
        res.status(500).json({
          message:
            "Error al llamar a la API externa. no se entregaron los datos correspondientes. - BLOQUE 3",
        });
      }
    } else {
      res.status(500).json({
        message:
          "Error al llamar a la API externa. no se entregaron los datos correspondientes. - BLOQUE 4",
      });
    }
  } catch (error) {
